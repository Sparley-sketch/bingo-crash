...
export async function POST(){
  // 1) admin gate via session (RLS-safe read of profiles)
  ...
  // 3) If a LIVE round already exists, just return it (idempotent Start)
  {
    ...
    if (live) {
      return NextResponse.json(
        { id: live.id, phase: live.phase, speed_ms: live.speed_ms, called: live.called },
        { headers: { 'Cache-Control': 'no-store' } }
      );
    }
  }

  // 4) Make sure no straggler lives remain
  await supabase.from('rounds').update({ phase: 'ended' }).eq('phase','live');

  // 5) Insert a new LIVE round (handle races with unique index)
  ...
  // 6) If unique violation (23505), another request won the race → return that live row
  if (error?.code === '23505' || /duplicate key value/.test(String(error?.message || ''))) {
    const { data: liveAgain, error: liveErr } = await supabase
      .from('rounds')
      .select('*')
      .eq('phase','live')
      .order('created_at',{ ascending:false })
    ...
    if (liveErr) {
      return NextResponse.json({ error: `start race: ${liveErr.message}` }, { status: 500 });
    }
    if (liveAgain) {
      return NextResponse.json(
        { id: liveAgain.id, phase: liveAgain.phase, speed_ms: liveAgain.speed_ms, called: liveAgain.called },
        { headers: { 'Cache-Control': 'no-store' } }
      );
    }
  }
  ...
  // 7) Any other error
  return NextResponse.json(
    { error: `round insert failed: ${error?.message || 'unknown'}` },
    { status: 500 }
  );
}
